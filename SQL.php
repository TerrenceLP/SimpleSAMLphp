...
class sspmod_sqlauth_Auth_Source_SQL extends sspmod_core_Auth_UserPassBase {
	...
	protected function login($username, $password) {
		require("/opt/bitnami/apps/wordpress/htdocs/sso/modules/sqlauth/phpass/PasswordHash.php");
		assert('is_string($username)');
		assert('is_string($password)');

		$db = $this->connect();
		// Just in case the hash isn't found we really should do something right about now
		// Retrieve the hash that you stored earlier
		$pwchk = $db->prepare("SELECT user_login, user_pass FROM bc_users WHERE user_login = :username;");
		$pwchk->bindValue(':username', $username, PDO::PARAM_STR);
		$pwchk->execute();
		$pwchk_result = $pwchk->fetch(PDO::FETCH_ASSOC);

		$stored_hash = $pwchk_result['user_pass'];
		...
		try {
			$res = $sth->execute(array('username' => $username, 'password' => $stored_hash));
		} catch (PDOException $e) {
			throw new Exception('sqlauth:' . $this->authId .
